...
from components import COMPONENTS, NIKKEI_225_TICKERS

st.set_page_config(page_title="日経225 SQ計算ツール", page_icon="📊", layout="wide")

st.title("📊 日経225 SQ計算ツール")
st.caption("ウィークリー・月次オプション対応 ｜ 構成銘柄の始値 × 株価換算係数 ÷ 除数")
...
st.success(f"✅ 構成銘柄 {len(NIKKEI_225_TICKERS)} 銘柄・株価換算係数を読み込み済み（components.py）")
...
with st.expander("🔧 実際のSQ値から除数を逆算する（除数確認用）"):
    st.caption("始値合計（換算後）が分かっている場合に、正しい除数を確認できます")
    col_x, col_y, col_z = st.columns(3)
    real_sq = col_x.number_input("実際のSQ値（円）", value=0.0, step=100.0, format="%.2f")
    sum_input = col_y.number_input("換算後始値合計（円）", value=0.0, step=1000.0, format="%.1f")
...
if st.button("🔄 始値を取得してSQを計算する", type="primary"):
    ...
    with st.spinner(f"{len(NIKKEI_225_TICKERS)}銘柄のデータを取得中...（約30秒かかります）"):
        try:
            tickers_str = " ".join(NIKKEI_225_TICKERS)
            raw = yf.download(
                tickers_str,
                period="2d",
                interval="1d",
    ...
    for ticker in NIKKEI_225_TICKERS:
        factor = COMPONENTS.get(ticker, 1.0)
        ...
        if pd.isna(price) or price is None or price == 0:
            ...
            raw_price = round(float(price), 1)
            adopted = round(raw_price * factor, 1)
            records.append({
                "コード": ticker.replace(".T", ""),
                "ティッカー": ticker,
                "始値": raw_price,
    ...
    total_adopted = fetched["採用株価"].sum()
    sq_value = total_adopted / divisor_input

    # ── SQ表示 ───────────────────────────────────────────────
    st.subheader("③ SQ算出結果")
    col_a, col_b, col_c, col_d = st.columns(4)
    col_a.metric("推計SQ値", f"{sq_value:,.2f} 円")
    col_b.metric("取得済銘柄数", f"{len(fetched)} / {len(NIKKEI_225_TICKERS)}")
    col_c.metric("採用株価合計", f"{total_adopted:,.1f} 円")
    ...
    st.caption(
        f"算出時刻：{datetime.now().strftime('%H:%M:%S')} ｜ "
        f"SQ = 採用株価合計（{total_adopted:,.1f}）÷ 除数（{divisor_input:.3f}）"
    )
    ...
    with tab1:
        st.dataframe(
            fetched[["コード", "始値", "換算係数", "採用株価"]].reset_index(drop=True),
    ...
    with tab2:
        if len(missing) > 0:
            st.dataframe(missing[["コード", "ティッカー", "換算係数"]].reset_index(drop=True), use_container_width=True)
        else:
...
st.divider()
st.caption(
    "**計算式**：SQ = Σ（始値 × 株価換算係数） ÷ 除数　"
    "（出所：日本経済新聞社 算出要領）　"
    "**注意**：本ツールはyfinanceの非公式データを使用しています。公式SQ値は大阪取引所の発表をご確認ください。"
)
